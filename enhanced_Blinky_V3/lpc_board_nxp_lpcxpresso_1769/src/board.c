 ...
 ****************************************************************************/
#define ONBOARD_USER_BUTTON_GPIO_PORT_NUM       2
#define ONBOARD_USER_BUTTON_GPIO_BIT_NUM        10
#define UP_BUTTON_GPIO_PORT_NUM                 0
#define UP_BUTTON_GPIO_BIT_NUM                  2
#define DOWN_BUTTON_GPIO_PORT_NUM               0
#define DOWN_BUTTON_GPIO_BIT_NUM                3
...
#define RED_LED_GPIO_BIT_NUM                    22  //RED_LED LED
#define GREEN_LED_GPIO_PORT_NUM                 3
#define GREEN_LED_GPIO_BIT_NUM                  25  //Green LED
#define BLUE_LED_GPIO_PORT_NUM                  3
#define BLUE_LED_GPIO_BIT_NUM                   26  //Blue LED
...
{
	/* Pin PIO0_22 is configuRED_LED as GPIO pin during SystemInit */
...
void Board_LED_Set(uint8_t LEDNumber, bool On)
{
	/* RED_LED LED */
	if (LEDNumber == 0) {
		Chip_GPIO_WritePortBit(LPC_GPIO, RED_LED_GPIO_PORT_NUM, RED_LED_GPIO_BIT_NUM, On);
	}
	/* Green LED */
	if (LEDNumber == 1) {
			Chip_GPIO_WritePortBit(LPC_GPIO, GREEN_LED_GPIO_PORT_NUM, GREEN_LED_GPIO_BIT_NUM, On);
		}
	/* Blue LED */
	if (LEDNumber == 2) {
			Chip_GPIO_WritePortBit(LPC_GPIO, BLUE_LED_GPIO_PORT_NUM, BLUE_LED_GPIO_BIT_NUM, On);
		}
}
...
bool Board_LED_Test(uint8_t LEDNumber)
{
	...
	if (LEDNumber == 0) {
		state = Chip_GPIO_ReadPortBit(LPC_GPIO, RED_LED_GPIO_PORT_NUM, RED_LED_GPIO_BIT_NUM);
	}

	if (LEDNumber == 1) {
			state = Chip_GPIO_ReadPortBit(LPC_GPIO, GREEN_LED_GPIO_PORT_NUM, GREEN_LED_GPIO_BIT_NUM);
		}
	if (LEDNumber == 2) {
			state = Chip_GPIO_ReadPortBit(LPC_GPIO, BLUE_LED_GPIO_PORT_NUM, BLUE_LED_GPIO_BIT_NUM);
		}
	return state;
}
...
/* Set up and initialize all requiRED_LED blocks and functions related to the
   board hardware */
...
{
	Chip_GPIO_WriteDirBit(LPC_GPIO, ONBOARD_USER_BUTTON_GPIO_PORT_NUM, ONBOARD_USER_BUTTON_GPIO_BIT_NUM, false);
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, UP_BUTTON_GPIO_PORT_NUM, UP_BUTTON_GPIO_BIT_NUM);
	Chip_IOCON_PinMux(LPC_IOCON, 0, 3, IOCON_MODE_PULLUP, IOCON_FUNC0);
	Chip_IOCON_PinMux(LPC_IOCON, 0, 2, IOCON_MODE_PULLUP, IOCON_FUNC0);
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, UP_BUTTON_GPIO_PORT_NUM, UP_BUTTON_GPIO_BIT_NUM);
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, DOWN_BUTTON_GPIO_PORT_NUM, DOWN_BUTTON_GPIO_BIT_NUM);
}

uint32_t Buttons_GetStatus(void)
{
	uint8_t ret = NO_BUTTON_PRESSED;
	if (Chip_GPIO_ReadPortBit(LPC_GPIO, ONBOARD_USER_BUTTON_GPIO_PORT_NUM, ONBOARD_USER_BUTTON_GPIO_BIT_NUM) == 0x00) {
		ret |= ONBOARD_USER_BUTTON;
	}
	return ret;
}
uint32_t UP_Button_GetStatus(void)
{
	uint8_t ret = NO_BUTTON_PRESSED;
	if (Chip_GPIO_GetPinState(LPC_GPIO, UP_BUTTON_GPIO_PORT_NUM, UP_BUTTON_GPIO_BIT_NUM) == 0x00) {
		ret |= UP_BUTTON_PRESSED;
	}
	return ret;
}
uint32_t DOWN_Button_GetStatus(void)
{
	uint8_t ret = NO_BUTTON_PRESSED;
	if (Chip_GPIO_GetPinState(LPC_GPIO, DOWN_BUTTON_GPIO_PORT_NUM, DOWN_BUTTON_GPIO_BIT_NUM) == 0x00) {
		ret |= DOWN_BUTTON_PRESSED ;
	}
	return ret;
}
